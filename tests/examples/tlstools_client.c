...
tlstools_connect(char *server,int port,long socket_sd,gnutls_session_t session, char *remote_logfile,int remote_logfile_len)
    ...
    */
    ...
    /* Responce will contain logfile: followed by slogd log file */
...
main (void)
{
  ...
  /* Connect to the server, and do our handshake */
  char remote_logfile[MAX_REMOTE_LOGFILE_LEN];
  memset (&remote_logfile, '\0', sizeof (remote_logfile));
  err=tlstools_connect(server,port,socket_sd,session, remote_logfile,MAX_REMOTE_LOGFILE_LEN); 
  if ( err<0 ) {
      goto end;
  }
  else if (err ==0 ) {
  ...
  /* Send randomd crap to session 
  *  tlstools_send returns bytes sent. 0 peer closed connection, <0 on error */
  tlstools_send(session,"line1\n",strlen("line1\n"));
  sleep (1);
  tlstools_send(session,"line2\n",strlen("line2\n"));
  sleep (1);
  tlstools_send(session,"line3\n",strlen("line3\n"));
  tlstools_send(session,"line4\n",strlen("line4\n"));
  
  /* End TLS session happily */
