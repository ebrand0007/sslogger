...
static gnutls_psk_server_credentials_t psk_cred;
...
initialize_tls_session (void)
{
  ...
  */
  ...
  if (use_tls_pks==1) {
    err = gnutls_credentials_set (session, GNUTLS_CRD_PSK, psk_cred);
    if (err != 0) goto failed;
  }
...
static int goDaemon(void) {
    ...
    switch (pid) {
        case 0:
        {
            ...
            nextpid = fork();
            int gf_got, gf_status = -1;
            switch (nextpid) {
                case 0:
                    log_info(1024,"Setting signals\n");
                   
                    //TODO: Can we move the signal peice to the where we acutually listen? 
                    /* child */
                   ...
                   //TODO: new in testing initscript daeomn handler, comented out below  
                   signal(SIGHUP, termination_handler);
                    ...
                    /* TODO: test Ignore signals from my children to prevent zobies */
                    return 0;
                case -1:
                    /* error forking */
                    return -1;
                default:
                    /* parent */
                    //TODO: working here
                    sleep (1); /* wait 1 sec for child to spawn */
                    /* We wait to make sure the next child forked successfully */
                    if ((gf_got = waitpid(nextpid, &gf_status, WNOHANG)) < 0 ||
                        gf_got != nextpid ||
                        gf_status != 0) {
                        return -1;
                    }
                    if (WIFEXITED(gf_status)) {
                      /* error: child exited for some reason */
                      return -1;
                    }    
                    fi
...
initializeGnuTLS (void)
{
  ...
  /* using tls_x590_auth */  
    if ( (use_tls_pks == 1) || (use_tls_cert ==1 ) ) { 
        ...
        if (use_tls_pks == 1 ) {
            if (debug>1) 
                log_info(1000,"Using TLS pks authentication\n");
            //TODO: ERRO CHECKING BELOW CMD         
            ...
            if (err < 0) {
                log_info(1000,"gnutls_psk_set_server_credentials_file: %s",
                        gnutls_strerror (err));
                        /*TODO: log this to syslog*/
                        return -1;
            }
            // not implemented in TLS v1.2
...
proccess_connection_fork(struct worker_pthread *worker) { 
   ...
   asprintf(&logfname,"%s/%s",log_dir,log_file);
    ...
    log_info(1200,"Connection from %s port %d\n",worker->fqdn,worker->cli_port);
    ...
    */
    ...
    if (strcmp(header,HEADER_LABEL)!=0) {
        ...
        if (debug>2) 
            log_info(1024,"Malformed header from :%s\n",worker->fqdn);
        goto cleanup;
    }
    if (debug>2)
    ...
    char *message;
    asprintf(&message,"host:%s; logfile:%s; %s",worker->fqdn, fname, logString);
